...
export function FindMe({ title = 'Find Me' }: {title?: string;}) {
  ...
  return (
    <div className="flex-1 flex flex-col bg-canvas">
      ...
      <div className="px-4 pt-3 pb-2 bg-white border-b hairline">
        <label className="sr-only" htmlFor="findme-search">
          What are you looking for?
        </label>
        <div className="flex items-center gap-2.5 border hairline rounded-lg px-3 tap bg-canvas">
          ...
          <input
            ...
            className="flex-1 bg-transparent txt-sm text-navy placeholder:text-muted outline-none" />
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar">
            ...
            {categories.map((c) => {
              ...
              return (
                <button
                  ...
                  className={`rounded-xl border px-2 py-3 flex flex-col items-center gap-1.5 text-center transition-colors duration-150 ease-out ${
                  on ?
                  'bg-navy text-white border-navy' :
                  'bg-white text-navy hairline hover:border-navy'}`
                  }>
        ...
        <div className="px-4 pb-6">
          <h2 className="txt-xs font-semibold tracking-[0.12em] uppercase text-muted mb-2">
            {category ? `Nearest ${catLabel}` : 'Nearest to you'}
          </h2>

          {results.length === 0 ?
          <Card className="p-4">
              <p className="txt-sm text-navy font-semibold">Nothing found</p>
              <p className="txt-sm text-muted">
                Try another category, or ask at the Information Desk on the middle concourse.
              </p>
            </Card> :
          ...
          <ul className="space-y-2">
            ...
            map((f, i) =>
            <li key={f.id}>
                    <Card className={`p-3.5 ${i === 0 ? 'border-2 border-navy' : ''}`}>
                      <div className="flex items-start gap-3">
                        <span
                    className="w-9 h-9 rounded-lg bg-slate-100 text-navy flex items-center justify-center shrink-0"
                    aria-hidden="true">
                        ...
                        <div className="flex-1 min-w-0">
                          {i === 0 &&
                    <p className="txt-xs font-semibold uppercase tracking-wider text-teal">
                              Nearest
                            </p>
                    }
                        ...
                        f.accessible ? 'text-teal' : 'text-muted'}`
                          ...
                          <div className="mt-1.5 flex items-center gap-2">
                            <span className="txt-xs font-medium text-setu-green">
                              {f.status}
                            </span>
