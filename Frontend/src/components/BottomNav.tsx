...
import { HomeIcon, MapIcon, RouteIcon, TrainFrontIcon, UserIcon } from 'lucide-react';
...
export function BottomNav() {
  ...
  return (
    <nav
      aria-label="Primary"
      className="shrink-0 bg-white border-t hairline px-1 pb-1 pt-1">
          ...
          return (
            <li key={id} className="flex-1">
              <button
                ...
                className={`tap w-full flex flex-col items-center justify-center gap-0.5 rounded-lg py-1 transition-colors duration-150 ease-out ${
                active ? 'text-navy' : 'text-muted hover:text-navy'}`
                }>
                
                <Icon className="w-[22px] h-[22px]" strokeWidth={active ? 2.2 : 1.7} />
                <span className={`txt-xs ${active ? 'font-semibold' : 'font-medium'}`}>
                  {label}
                </span>
              </button>
            </li>);
